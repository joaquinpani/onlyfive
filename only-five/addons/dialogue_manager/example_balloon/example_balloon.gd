...
## =========================================================
## TUS VARIABLES (ASÍGNALAS EN EL INSPECTOR DE GODOT)
## =========================================================
@export_group("Nodos de la Escena")
## Arrastra aquí el nodo de tu Sprite2D o TextureRect
@export var sprite_personaje: CanvasItem 
## Arrastra aquí tu nodo AudioStreamPlayer
...
@export var sonidos_p2: Array[AudioStream]
...
@export_group("Opciones de Dialogue Manager")
...
func _ready() -> void:
	...
	# Conectamos la señal para reproducir el sonido al escribir letras
...
#region Lógica Custom de Personajes y Sonidos

func actualizar_personaje() -> void:
	if not sprite_personaje: 
		return # Previene crasheos si no asignaste el nodo
		
	if GameManager.personaje_actual == 0:
		sprite_personaje.visible = false
	else:
		sprite_personaje.visible = true
		if texturas_personajes.size() >= GameManager.personaje_actual:
			var textura = texturas_personajes[GameManager.personaje_actual - 1]
			# Soporta que tu nodo sea Sprite2D o TextureRect
			if sprite_personaje is Sprite2D:
				sprite_personaje.texture = textura
			elif sprite_personaje is TextureRect:
				sprite_personaje.texture = textura


func _on_dialogue_label_spoke(letter: String, letter_index: int, speed: float) -> void:
	if not letter.strip_edges().is_empty():
		reproducir_sonido_personaje()


func reproducir_sonido_personaje() -> void:
	if not audio_stream_player:
		return
		
	var sonidos_activos: Array[AudioStream] = []
	
	match GameManager.personaje_actual:
		1: sonidos_activos = sonidos_p1
		2: sonidos_activos = sonidos_p2
